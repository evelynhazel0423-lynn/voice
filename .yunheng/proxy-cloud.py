...
# 部署：scp 到云机，nohup python3 proxy.py & （端口 8899，0.0.0.0）
...
# refreshToken 从同目录文件读（云端不存 CLI 配置）
RT_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "refresh-token.txt")
LOCK = threading.Lock()
...
def bootstrap():
    ...
    try:
        rt = open(RT_FILE).read().strip()
        c = {"token": "", "refresh": rt, "exp": 0}
        save_cache(c); return c
    except Exception as e:
        print("bootstrap fail:", e, flush=True)
